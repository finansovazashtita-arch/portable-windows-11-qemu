...
- **Autonomous Global Multi-Entity Tax & VAT Engine**: `src/audit/global_tax_engine.py`, `src/integration/hmrc_mtd_adapter.py`, `src/audit/us_sales_tax_engine.py`, `src/audit/swiss_estv_tax_engine.py` providing multi-jurisdiction tax calculation, filing, and double-entry mapping across Bulgaria (НАП), EU (VIES/OSS), UK (HMRC MTD), US (State Sales Tax), and Switzerland (ESTV).
...
| 84 | Predictive AI Advisory & Executive Decision Engine | Multi-scenario financial trajectory simulator, prescriptive C-level recommendations, double-entry journal advice, working capital CCC optimizer & tax strategy advisor | M77 | DONE |
