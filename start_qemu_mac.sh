#!/usr/bin/env bash
# start_qemu_mac.sh
# QEMU Launcher script for Windows 11 on macOS (Apple Silicon / Intel Mac).
...
if sysctl -n hw.optional.arm64 2>/dev/null | grep -q 1; then
    echo "[INFO] Detected Apple Silicon (ARM64). Using TCG instruction emulation for x86_64 guest."
    ACCEL_FLAG="-accel tcg"
else
    echo "[INFO] Detected Intel Mac. Testing HVF acceleration..."
...
echo "[INFO] Starting QEMU Windows 11 VM..."
